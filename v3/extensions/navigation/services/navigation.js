...
const buildNestedStructure = (entities, id = null, field = "parent") =>
  entities
    .filter(
      (entity) =>
        entity[field] === id ||
        (isObject(entity[field]) && entity[field].id === id)
    )
    .map((entity) => ({
      ...entity,
...
const templateNameFactory = async (items, strapi) => {
  ...
  const relatedMap = flatRelated.reduce((acc, curr = {}) => {
    // console.debug('cur?', curr, acc)
    if (curr !== null) {
      if (!curr.__contentType) {
        curr.__contentType = "section";
      }
  ...
  const responses = await Promise.all(
    Object.entries(relatedMap).map(([contentType, ids]) =>
      strapi
        .query(contentType)
        .find({ id_in: ids })
...
module.exports = {
  ...
  config: async () => {
    ...
    const additionalFields = get(
      strapi.config,
      "custom.plugins.navigation.additionalFields",
      []
    );
    let extendedResult = {};
    const result = {
      ...
      allowedLevels: get(
        strapi.config,
        "custom.plugins.navigation.allowedLevels"
      ),
      additionalFields,
    };
  ...
  configContentTypes: () =>
    Object.keys(strapi.contentTypes)
      .filter(
        (key) =>
          excludedContentTypes.filter((ect) => key.includes(ect)).length === 0
      )
      .map((key) => {
  ...
  getById: async (id) => {
    ...
    return {
      ...sanitizeEntity(entity, { model: masterModel }),
  ...
  renderType: async (
    ...
    if (entity && entity.id) {
      ...
      // console.debug('getTemplateName', getTemplateName);
      ...
      switch (type) {
        ...
        case renderType.RFR:
          const itemParser = (item, path = "", field) => {
            ...
            const slug = isString(parentPath)
              ? slugify(
                  (first(parentPath) === "/"
            ...
            return {
              ...
              slug:
                !slug && item.uiRouterKey ? slugify(item.uiRouterKey) : slug,
              external: isExternal,
              // related: (isExternal || !firstRelated) ? undefined : {
              //   ...firstRelated,
              //   __templateName: getTemplateName(firstRelated.__contentType, firstRelated.id),
              // },
              audience: !isEmpty(item.audience)
                ? item.audience.map((aItem) => aItem.key)
                : undefined,
              items: isExternal
                ? undefined
                : service.renderTree(
  ...
  renderTree: (
    ...
    var result = items.filter(
      (item) =>
        item[field] === id || (isObject(item[field]) && item[field].id === id)
    );
  ...
  renderRFR: (items, parent = null, parentNavItem = null) => {
    ...
    items.forEach((item) => {
      ...
      if (!parent) {
        ...
        const navLevel = navItems.filter(
          (navItem) =>
            navItem.type === navigationItem.type.INTERNAL.toLowerCase()
        );
        if (!isEmpty(navLevel))
          nav = {
            ...nav,
      ...
      if (!isEmpty(itemChilds)) {
        ...
        const { pages: nestedPages } = service.renderRFR(
          itemChilds.filter(
            (child) => child.type === navigationItem.type.INTERNAL
          ),
          itemPage.id,
          itemNav
        );
  ...
  renderRFRPage: (item, parent) => {
    ...
    const { collectionName } =
      find(contentTypes, (ctItem) => ctItem.name === contentType) || {};
    return {
      id: uiRouterKey,
      title,
      templateName: __templateName,
      related:
        type === navigationItem.type.INTERNAL
          ? {
              contentType,
              collectionName,
              id,
  ...
  removeBranch: async (items = []) => {
    ...
    return Promise.all(
      items
        ...
        .map(async (item) => {
          const { id } = item;
          await strapi.query(itemModel.modelName, pluginName).delete({ id });
          return !isEmpty(item.items) ? service.removeBranch(item.items) : null;
        })
    );
  },
  ...
  analyzeBranch: async (items = [], masterEntity = null, parentItem = null) => {
    const { pluginName, itemModel, service } = extractMeta(strapi.plugins);
    const needToCreate = items.filter(
      (item) => isNil(item.id) && !item.removed
    );
    const needToAnalyzeAndUpdate = items.filter(
      (item) => !isNil(item.id) && !item.removed
    );
    const needToRemove = items.filter(
      (item) => !isNil(item.id) && item.removed
    );
    ...
    return Promise.all([
      ...
      Promise.all(
        needToAnalyzeAndUpdate.map(async (item) => {
          const { id, updated, parent, master, related, items, ...params } =
            item;
          let currentItem;
          if (updated) {
            const relatedItem =
            ...
            await strapi
              .query(itemModel.modelName, pluginName)
              .update({ id }, { related: [] }); // clearing the relation to get it updated properly and not duplicate _morph records
