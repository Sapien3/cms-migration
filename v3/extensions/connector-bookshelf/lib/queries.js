...
module.exports = function createQueryBuilder({ model, strapi }) {
  ...
  function find(params, populate, { transacting } = {}) {
    ...
    // console.log('bookshelf query', model.uid, filters, stackTrace.get().toString());
    // console.log('bookshelf query', model.uid, filters);
    return model
      .query(query)
      .fetchAll({
      ...
      .then((results) => {
        ...
        if ("sort" in filters || !("where" in filters)) {
          return rows;
        }
        // console.log("filters", filters);
        const idCond = filters.where.filter((where) => {
          return where.field == "id" && where.operator == "in";
        });
        // console.log("idcond", idCond);
        if (idCond.length == 0) {
          return rows;
        }
        // reorder rows to id_in
        const list = [];
        const idRows = {};
        for (var row of rows) {
          idRows[row.id.toString()] = row;
        }

        // console.log("queried", idCond[0].value, idRows);
        for (var id of idCond[0].value) {
          console.log("loop", id);
          if (!id in idRows) {
            console.log("id missing, skipped", id);
            continue;
          }
          list.push(idRows[id]);
        }
  ...
  async function update(params, attributes, { transacting } = {}) {
    ...
    const relations = pickRelations(attributes);
    const data = selectAttributes(attributes);
    // console.log("update", model.uid, params, data);

    const runUpdate = async (trx) => {
      // console.log("runUpdate", model.uid, params, data);
      const updatedEntry =
      ...
      if (Object.keys(relations).length > 0) {
        // console.debug('runUpdate updateRelations', model.uid, entry.id, relations);
        return model.updateRelations(
          { id: entry.id, values: relations },
          { transacting: trx }
        );
  ...
  async function updateComponents(entry, attributes, { transacting }) {
    ...
    const updateOrCreateComponentAndLink = async ({
      ...
      if (_.has(value, componentModel.primaryKey)) {
        // console.debug("updatecomp", componentModel.uid, value);
        return strapi
          .query(componentModel.uid)
          .update(
            {
