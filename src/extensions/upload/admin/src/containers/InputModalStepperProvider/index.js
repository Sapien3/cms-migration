...
const InputModalStepperProvider = ({
  ...
  const [reducerState, dispatch] = useReducer(reducer, initialState, (state) =>
    init({
      ...
      initialSelectedFiles: Array.isArray(selectedFiles)
        ? selectedFiles
        : [selectedFiles],
      ...
      filesToUpload: initialFilesToUpload
        ? createNewFilesToUploadArray(initialFilesToUpload).map(
            (file, index) => ({
              ...file,
              originalIndex: index,
            })
  ...
  const getFilters = (filtersToOmit = []) => {
    ...
    const searchParams = generateSearchFromFilters(
      compactedParams,
      filtersToOmit
    );
