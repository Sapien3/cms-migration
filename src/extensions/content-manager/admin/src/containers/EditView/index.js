...
const EditView = (props) => {
  const { isSingleType, goBack, layout, slug, state, id, origin } = props;
  ...
  const handleClick = async (e) => {
    e.persist();
    console.log("id", id);
    const res = await axios.get(`https://cms.mediatechservice.com/getarticleUrl/${id}`);
    const url = res.data;
    console.log("url: ", `https://web.mediatechservice.com${url}`);
    if (!url) return alert("article not found");
    window.open(`https://web.mediatechservice.com/${url}`, "_blank");
  };
            ...
            <Container className="container-fluid">
              ...
              <div className="row" style={{ paddingTop: 3 }}>
                ...
                <div className="col-md-12 col-lg-3">
                  {layout.contentType.uid == "application::article.article" && (
                    <Button onClick={handleClick}>Preview Article</Button>
                  )}
                  <InformationCard />
                  <Padded size="smd" top />
